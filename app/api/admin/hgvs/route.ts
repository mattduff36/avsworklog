  ...
  try {
    ...
    try {
      ...
      const { error: maintenanceCleanupError } = await admin
        ...
        .delete()
        .eq('hgv_id', data.id);
      const { error: assetCleanupError } = await admin.from('hgvs').delete().eq('id', data.id);
      if (maintenanceCleanupError || assetCleanupError) {
        throw new AssetServiceError('Failed to roll back HGV after service setup failed', 500);
      }
