...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tasksError) {
      console.error('Error fetching recently completed defect tasks:', tasksError);
      return NextResponse.json({ error: 'Failed to fetch completed defects' }, { status: 500 });
    }

    const recentlyCompletedMap = buildRecentCompletedDefectMap(tasks || [], {
      lookbackDays,
    });
    ...
    console.error('Error in recent-completed-defects endpoint:', error);
