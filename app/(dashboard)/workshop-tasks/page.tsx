...
export default function WorkshopTasksPage() {
  ...
  const getStatusIcon = (status: string) => status === 'completed' ? <CheckCircle2 className="h-5 w-5 text-green-400" /> : status === 'logged' ? <Clock className="h-5 w-5 text-blue-400" /> : status === 'on_hold' ? <Pause className="h-5 w-5 text-purple-400" /> : <AlertTriangle className="h-5 w-5 text-amber-400" />;
