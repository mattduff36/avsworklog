...
function getStatusBadgeClass(status: string | null | undefined): string {
  ...
  return 'border-avs-yellow/30 bg-avs-yellow/10 text-avs-yellow';
}
...
function SummaryCard({ label, value, helper, icon, accent = 'slate' }: SummaryCardProps) {
  const accentClass = {
    yellow: 'border-avs-yellow/30 bg-avs-yellow/10 text-avs-yellow',
    ...
    <AppPageShell width="wide">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_21rem]">
        ...
        <div className="space-y-4">
          ...
          {item.quote_id ? (
            <Button asChild className="w-full bg-avs-yellow font-semibold text-slate-950 hover:bg-avs-yellow/90">
