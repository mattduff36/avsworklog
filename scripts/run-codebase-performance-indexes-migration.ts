...
const { Client } = pg;
const MIGRATION_FILE = 'supabase/migrations/20260324_codebase_performance_indexes.sql';
const EXPECTED_INDEXES = [
  'idx_hgv_inspections_hgv_id_inspection_date_desc',
  'idx_plant_inspections_plant_id_inspection_date_desc',
  'idx_suggestions_status_created_at_desc',
  'idx_suggestions_created_by_created_at_desc',
  'idx_quotes_is_latest_version_created_at_desc',
  'idx_quotes_customer_id_is_latest_version_created_at_desc',
  'idx_error_reports_created_by_created_at_desc',
] as const;
...
async function runMigrationWithConnection(connectionString: string) {
  ...
  try {
    ...
    console.log('Codebase performance indexes applied and verified.');
  } finally {
    await client.end();
  }
}
...
async function main() {
  ...
  for (const connectionString of connectionCandidates) {
    try {
      ...
      if (
        (!message.includes('MaxClientsInSessionMode') && !message.includes('Unable to check out connection from the pool')) ||
        connectionString === connectionCandidates[connectionCandidates.length - 1]
      ) {
        throw error;
      }
      console.warn('Primary connection is saturated, retrying with fallback connection...');
    }
  }
