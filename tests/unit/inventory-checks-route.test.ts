...
import { INVENTORY_SERVICE_CHECKLIST_ITEMS } from '@/lib/checklists/inventory-service-checklist';
...
function buildChecklist(status: 'ok' | 'attention' | 'na' = 'ok') {
  return INVENTORY_SERVICE_CHECKLIST_ITEMS.map((item) => ({
    ...item,
    status,
...
describe('inventory check route', () => {
  ...
  it('stores canonical checklist data and derives the overall status', async () => {
    ...
    const response = await POST(
      buildRequest({
        checked_at: '2026-06-01',
        checklist_version: 'minor-plant-equipment-service-record-v1',
        checklist_items: checklist,
      }),
      { params: Promise.resolve({ id: 'item-1' }) },
    );
