...
describe('TEE V2.4 lineage budget and disposition', { timeout: 50_000 }, () => {
  it('T-V24-FIRST-AT-0 / T-V24-CLOSURE-AFTER-FIRST-FAIL / T-V24-SECOND-FAIL-ROUTING / T-V24-FIRST-REJECT-AT-2 / T-V24-CLOSURE-REJECT-AT-2', () => {
    ...
    initWorkstream(repoRoot, 'ws_v24_budget', baseCommit);
    expect(readProtocolRecord(repoRoot, 'ws_v24_budget')?.failedPremiumReviewCount).toBe(0);
    failFirstThenClosure(repoRoot, 'ws_v24_budget');
    const firstAgain = applyProtocolTransition({
      repoRoot,
      command: 'review-start',
      workstreamId: 'ws_v24_budget',
      pass: 'first',
    });
  ...
  it('T-V24-COSMETIC-SPLIT-INHERITS / T-V24-NARROWER-SPLIT-INHERITS / T-V24-FIXDELTA-SPLIT-INHERITS / T-V24-PREFLIGHT-NO-REOPEN / T-V24-REPEATED-SPLIT-NO-MINT / T-V24-AVS-REGRESSION-PATTERN', () => {
    const repoRoot = makeTempRoot('split');
    const baseCommit = initGitRepo(repoRoot);
    initWorkstream(repoRoot, 'ws_v24_root', baseCommit);
    failFirstThenClosure(repoRoot, 'ws_v24_root');

    const cosmetic = applyProtocolTransition({
      repoRoot,
      command: 'split',
      workstreamId: 'ws_v24_root',
    ...
    const narrower = applyProtocolTransition({
      repoRoot,
      command: 'split',
      workstreamId: 'ws_v24_cosmetic',
      newWorkstreamId: 'ws_v24_narrower',
  ...
  it('T-V24-ROUTE-NO-APPROVE / T-V24-ROUTE-NO-BIND-HEAD / T-V24-ROUTE-NO-FINALISE-READY / T-V24-EXHAUSTED-BLOCKS-FINALISE / T-V24-REMOVAL-RESOLVES-WITHOUT-FINALISE / T-V24-SUPERSEDE-REQUIRES-GIT', { timeout: 20000 }, () => {
    ...
    initWorkstream(repoRoot, 'ws_v24_route', baseline);
    failFirstThenClosure(repoRoot, 'ws_v24_route');
    const exhausted = readProtocolRecord(repoRoot, 'ws_v24_route')!;
    ...
    const routed = readProtocolRecord(repoRoot, 'ws_v24_route')!;
    ...
    expect(readiness.lineages.find((row) => row.workstreamId === 'ws_v24_route')?.role).toBe(
      'non_release_disposition'
    );
    expect(readiness.lineages.find((row) => row.workstreamId === 'ws_v24_route')?.phase).not.toBe(
      'finalised'
    );
  });
  ...
  it('T-V24-REHOME-REQUIRES-INDEPENDENT-GIT / T-V24-SPLIT-CHILD-NOT-REHOME / T-V24-NEW-ID-NO-FRESH-BUDGET / T-V24-SUCCESSOR-RETAINS-PREDECESSOR / T-V24-ILLEGAL-ATTEMPTS-AUDIT-ONLY', { timeout: 20000 }, () => {
    ...
    initWorkstream(repoRoot, 'ws_v24_pred', baseline);
    failFirstThenClosure(repoRoot, 'ws_v24_pred');
    const split = applyProtocolTransition({
      repoRoot,
      command: 'split',
      workstreamId: 'ws_v24_pred',
      newWorkstreamId: 'ws_v24_child',
      narrowerPartition: true,
    });
    expect(split.ok).toBe(true);
    const childRecord = readProtocolRecord(repoRoot, 'ws_v24_child')!;
    ...
    const sameContext = applyProtocolTransition({
      repoRoot,
      command: 'init',
      workstreamId: 'ws_v24_newid',
      baseCommit: baseline,
      sourceWorkstreamIds: ['ws_v24_pred'],
    });
    expect(sameContext.ok).toBe(true);
    expect(readProtocolRecord(repoRoot, 'ws_v24_newid')?.failedPremiumReviewCount).toBeGreaterThanOrEqual(
      2
    );
    expect(
      applyProtocolTransition({
    ...
    exhaustSourceWorkstream(repoRoot, 'ws_v24_source', baseline);
    ...
    const successorRecord = createEmptyProtocolRecord({
      ...
      rehomeProvenance: declaredRehome(baseline, 'successor', blocked, `${repoRoot}#main`, {
        ...
        sourceReviewWorkstreamId: 'ws_v24_source',
      }),
    });
    ...
    const bound = readProtocolRecord(repoRoot, 'ws_v24_successor');
    expect(bound?.rehomeProvenance?.status).toBe('bound');
    expect(bound?.rehomeProvenance?.predecessorRootWorkstreamId).toBe('ws_96e9f347f9da5b8f');
    ...
    const audit: WorkflowProtocolRecord = {
      ...readProtocolRecord(repoRoot, 'ws_v24_pred')!,
    };
  ...
  it('FD-GIT / FD-REHOME / FD-LINEAGE / FD-VERIFY first-review blocker family', { timeout: 20000 }, () => {
    ...
    const baseline = initGitRepo(repoRoot);
    const impl = commitFile(repoRoot, 'impl.ts', 'impl');
    initWorkstream(repoRoot, 'ws_v24_fix', baseline);
    failFirstThenClosure(repoRoot, 'ws_v24_fix');
    ...
    const routed = readProtocolRecord(repoRoot, 'ws_v24_fix')!;
    ...
    initWorkstream(inheritRoot, 'ws_v24_inherited_parent', inheritBase);
    const parent = readProtocolRecord(inheritRoot, 'ws_v24_inherited_parent')!;
    ...
    const inheritedSplit = applyProtocolTransition({
      ...
      command: 'split',
      workstreamId: 'ws_v24_inherited_parent',
      newWorkstreamId: 'ws_v24_inherited_child',
    });
    ...
    initWorkstream(verifyRoot, 'ws_v24_verify', verifyBase);
    ...
    const contract = createDefaultPlanContract({
      workstreamId: 'ws_v24_verify',
      taskId: 'task-v24-verify',
      taskType: 'change',
      risk: 'high',
      rationale: 'plan-bound preflight',
      fallbackEscalation: 'escalate',
      requiredTests: [{ id: 'T-V24-FIRST-AT-0', status: 'completed' }],
      independentReviewReasons: ['workflow-protocol-persistence'],
    });
    writeFileSync(
    ...
    const verifyRecord = readProtocolRecord(verifyRoot, 'ws_v24_verify')!;
    verifyRecord.planPath = path.join(planDir, 'plan.md');
    writeProtocolRecord(verifyRoot, verifyRecord);
    const verifyManifest = writePassingManifest(verifyRoot, 'ws_v24_verify', 'preflight');
