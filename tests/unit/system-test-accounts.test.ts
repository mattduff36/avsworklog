...
describe('system test account filtering', () => {
  it('matches dedicated testsuite account emails', () => {
    expect(isHiddenSystemTestAccountEmail('testsuite-admin@squiresapp.test')).toBe(true);
    expect(isHiddenSystemTestAccountEmail('testsuite-manager@squiresapp.test')).toBe(true);
    expect(isHiddenSystemTestAccountEmail('testsuite-employee@squiresapp.test')).toBe(true);
    expect(isHiddenSystemTestAccountEmail('manager@avsworklog.test')).toBe(true);
  ...
  it('filters hidden accounts but keeps normal and placeholder users', () => {
    const visible = filterHiddenSystemTestAccounts([
      { id: '1', email: 'testsuite-admin@squiresapp.test', full_name: 'Testsuite Admin' },
