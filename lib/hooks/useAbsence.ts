...
import { evaluateAbsenceDeleteGuard } from '@/lib/utils/absence-delete-guard';
...
export function useDeleteAbsence() {
  ...
  const isFullAccessAdmin = Boolean(isAdmin || isSuperAdmin);
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      const deleteGuard = evaluateAbsenceDeleteGuard({
        date: existingAbsence.date,
        status: existingAbsence.status,
        isBankHoliday: existingAbsence.is_bank_holiday,
        autoGenerated: existingAbsence.auto_generated,
        bulkBatchId: existingAbsence.bulk_batch_id,
        hasEditScope: true,
        isFullAccessAdmin,
      });
      if (!deleteGuard.allowed) {
        throw new Error(deleteGuard.reason || 'This absence cannot be deleted.');
      }
