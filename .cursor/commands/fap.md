...
5. If a safely repairable deterministic step fails, inspect `docs_private/automation/finalise-last-failure.json`, fix it, and run `npm run finalise:repair`. Repeat only that targeted check until stable, then run `npm run finalise:push` once for closure so the original push intent is preserved.
