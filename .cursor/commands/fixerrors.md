...
2. If the snapshot is empty, do not run cleanup. Otherwise obtain explicit confirmation: production error logs have been exported; only the exact verified snapshot rows and their registered dependent diagnostic alerts will be cleared; newer errors remain; the exported artifact is retained.
...
6. The export/confirmation/cleanup sequence is sequential. After clustering, apply the existing V2.1 Multitask advisory only when multiple substantial clusters are genuinely independent.
