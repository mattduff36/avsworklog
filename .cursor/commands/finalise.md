...
5. For a safely repairable step, inspect `docs_private/automation/finalise-last-failure.json`, fix the issue, and run `npm run finalise:repair`. Repeat only that targeted check until stable, then run `npm run finalise` once for closure.
6. Never use generic repair for migration/database, commit, push, unknown, stale, or destructive steps. Stop for merge conflicts, missing access/environment, production-data risk, PRD drift, or broad ambiguous fixes.
