...
2. Check `Collector wiring`, `Events`, `Latest event`, and `Telemetry confidence` first. If wiring is incomplete, repair the missing project hook component. If confidence is `none` or `warming-up`, do not infer workflow trends; confirm the project hook appears in Cursor Settings → Hooks, complete a substantive task with one valid V4 parent marker, then rerun diagnostics.
