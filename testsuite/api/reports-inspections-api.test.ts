...
describe('Reports Inspections — Auth Guards', () => {
  ...
  it('GET /api/reports/absence-leave/bookings returns 401 without auth', async () => {
    const res = await fetch(`${BASE_URL}/api/reports/absence-leave/bookings?dateFrom=2026-01-01&dateTo=2026-01-31`);
    expect([401, 403]).toContain(res.status);
  });

  it('GET /api/reports/absence-leave/allowance-totals returns 401 without auth', async () => {
    const res = await fetch(`${BASE_URL}/api/reports/absence-leave/allowance-totals?dateFrom=2026-01-01&dateTo=2026-01-31`);
    expect([401, 403]).toContain(res.status);
  });
