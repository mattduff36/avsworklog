...
BEGIN
  ...
  WHERE il.is_active = TRUE
    ...
    AND (
      COALESCE(cardinality(p_location_types), 0) = 0
      OR il.location_type = ANY(p_location_types)
    )
    AND (
      v_search = ''
...
COMMENT ON FUNCTION public.inventory_search_locations(TEXT, BOOLEAN, INTEGER, INTEGER, TEXT[]) IS
  'Returns a deterministic page of active Inventory locations with multi-field search, optional location-type filter, and total count.';
